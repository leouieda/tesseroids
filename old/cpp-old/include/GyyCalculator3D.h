#ifndef _GYYCALCULATOR3D_H_
#define _GYYCALCULATOR3D_H_
...
class GyyCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GyyCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GyyCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Gyy <spherical> component of the Gravity Gradient Tensor using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gyy <spherical> component of gravity gradient tensor <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = ( rl_2*cosPhil*( ((((3*rrl*rrl*cosPsiLamb*cosPsiLamb)/(l_3*l*l)) - ((rrl*cosPhi*cosPhil*cosLambLambl)/(l_3))) + (r*cosPhi*cosPhi*(((rl*cosPsi) - r) / (l_3))) - (cosPhi*sinPhi*((rrl*(cosPhi*sinPhil - sinPhi*cosPhil*cosLambLambl))/(l_3)))) / (r_2*cosPhi*cosPhi) ) ); 
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GyyCalculator3D::si2eotvos = 1000000000;
