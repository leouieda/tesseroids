#ifndef _GXYCALCULATOR3D_H_
#define _GXYCALCULATOR3D_H_
...
class GxyCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GxyCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GxyCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Gxy <spherical> component of the Gravity Gradient Tensor using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gxy <spherical> component of gravity gradient tensor <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = rl_2*cosPhil*( ((rrl_l3*((3*r*rl*cosPsiLamb*cosPsiPhi)/(l*l) + (sinPhi*cosPhil*sinLambLambl))) + ((sinPhi/cosPhi)*(rrl_l3*cosPsiLamb))) / (r_2*cosPhi) ); 
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GxyCalculator3D::si2eotvos = 1000000000;
