#ifndef _POTENTIALCALCULATOR3D_H_
#define _POTENTIALCALCULATOR3D_H_

/* ****************************************************************************************** */
/* ****************************** PotentialCalculator3D ************************************* */
/* ****************************************************************************************** */
/* ******** Calculate the Gravity Potential Field using 3D Gauss-Legendre Quadrature ******** */
/* ****************************************************************************************** */
...
class PotentialCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        PotentialCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        PotentialCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Spherical Potential Field using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("spherical gravity potential <3D GLQ>");
                throw ex;
            }
            ...
            /* Aux variables to calculate KV */
            double rl, phil, lambl,
                   rl_2, 
                   cosPhil,
                   cosPsi,
                   l,
            ...
            lonStep = (E - W) / (nlon-1);
            latStep = (N - S) / (nlat-1);
            for(phi=S, clat=0; clat<nlat; phi+=latStep, clat++){
                for(lamb=W, clon=0; clon<nlon; lamb+=lonStep, clon++){
            ...
            for(e=0; e < modSize; e++){
                ...
                scaleFactor = ((eN-eS)*(eE-eW)*(eTop-eBottom)*mod[e].getDensity()*G) / 8;
                             
                /* Calculte the field caused by this element on the data grid */
                for(phi=Srad, clat=0; clat<nlat; phi+=latStep, clat++){
                    for(lamb=Wrad, clon=0; clon<nlon; lamb+=lonStep, clon++){
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    /* Sum this KV with the respective weights */ 
