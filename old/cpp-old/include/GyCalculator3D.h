#ifndef _GYCALCULATOR3D_H_
#define _GYCALCULATOR3D_H_
...
class GyCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GyCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GyCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Longitudinal <spherical> Component of the Gravity Field using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("longitudinal <spherical> component of gravity <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = (rl_2*cosPhil*(-1)*rl*cosPhil*sin(lamb - lambl)) / (l*l*l);
...
/* 1 m/s^2 = 10^5 mGal */
double GyCalculator3D::si2mgal = 100000;
