#ifndef _GZCALCULATOR2D_H_
#define _GZCALCULATOR2D_H_

/* ****************************************************************************************** */
/* ********************************* GzCalculator2D ***************************************** */
/* ****************************************************************************************** */
/* *************** Calculate the Vertical (gz) Component of the Gravity Field *************** */
...
class GzCalculator2D: public FieldCalculator {
    ...
    public:
        ...
        GzCalculator2D(int newLatO, int newLonO){
            latOrder = newLatO;
            lonOrder = newLonO;        
            boundsSet = false;
            hightSet = false;
        ...
        GzCalculator2D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO){            
            latOrder = newLatO;
            lonOrder = newLonO;        
            W = newW;
            E = newE;
        ...
        /* Calculate the Vertical Component of the Gravity Field using analytical integration */
        ...
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("vertical <spherical> component of gravity <2D GLQ>");
                throw ex;
            }
                    ...
                    for(lamb=Wrad, clon=0; /*lon<=Erad && */clon<nlon; lamb+=lonStep, clon++){
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){ 
                                ...
                                KV = ( (cosPhil / r)*( (r1*l1) - (r2*l2) - (3*r*cosPsi*(l2-l1)) - (r_2*((3*cosPsi*cosPsi)-1)*ln) + ((r2*r2_2)/l2) - ((r1*r1_2)/l1) ) );
...
/* Conversion factor */
/* 1 m/s^2 = 10^5 mGal */
double GzCalculator2D::si2mgal = 100000;
