#ifndef _GXZCALCULATOR2D_H_
#define _GXZCALCULATOR2D_H_
...
class GxzCalculator2D: public FieldCalculator {
    ...
    public:
        ...
        GxzCalculator2D(int newLatO, int newLonO){
            latOrder = newLatO;
            lonOrder = newLonO;        
            boundsSet = false;
            hightSet = false;
        ...
        GxzCalculator2D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO){            
            latOrder = newLatO;
            lonOrder = newLonO;        
            W = newW;
            E = newE;
        ...
        /* Calculate the Gxz <spherical> component of the Gravity Gradient Tensor using analytical integration */
        ...
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gxz <spherical> component of gravity gradient tensor <2D GLQ>");
                throw ex;
            }
                    ...
                    for(lamb=Wrad, clon=0; /*lon<=Erad && */clon<nlon; lamb+=lonStep, clon++){
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){  
                                ...
                                KVphir = (cosPsiPhi/r)*((rr1_l1*r1*r1_2/l1_2) - (rr2_l2*r2*r2_2/l2_2) + rr1_l1*r1 - rr2_l2*r2 + 3*r*sumt3 + 6*r_2*cosPsi*ln + r_2*r*cosPsi_2_1*sumt6);
                                KVphi = 0.5*cosPsiPhi*(rr1_l1*r1 - rr2_l2*r2 + 3*r*sumt3 + 6*r_2*cosPsi*ln + r_2*r*cosPsi_2_1*sumt6);
                                
                                KV = cosPhil*( ((KVphi/r) - KVphir) / (r) );
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GxzCalculator2D::si2eotvos = 1000000000;
