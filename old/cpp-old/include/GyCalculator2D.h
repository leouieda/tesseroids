#ifndef _GYCALCULATOR2D_H_
#define _GYCALCULATOR2D_H_

/* ****************************************************************************************** */
/* ********************************** GyCalculator2D **************************************** */
/* ****************************************************************************************** */
/* *********** Calculate the Longitudinal <gy> Component of the Gravity Field *************** */
...
class GyCalculator2D: public FieldCalculator {
    ...
    public:
        ...
        GyCalculator2D(int newLatO, int newLonO){
            latOrder = newLatO;
            lonOrder = newLonO;        
            boundsSet = false;
            hightSet = false;
        ...
        GyCalculator2D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO){            
            latOrder = newLatO;
            lonOrder = newLonO;        
            W = newW;
            E = newE;
        ...
        /* Calculate the longitudinal <spherical> Component of the Gravity Field using analytical integration */
        ...
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("longitudinal <spherical> component of gravity <2D GLQ>");
                throw ex;
            }
            ...
            for(e=0; e < modSize; e++){
                ...
                scaleFactor = (si2mgal*(eN-eS)*(eE-eW)*mod[e].getDensity()*G) / 4;
                    ...
                    for(lamb=Wrad, clon=0; /*lon<=Erad && */clon<nlon; lamb+=lonStep, clon++){
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){ 
                                ...
                                KV = ((cosPsiLamb*cosPhil)/(2*r*cosPhi))*( r*(r1*r1l1 - r2*r2l2) + (3*r*(l2 - l1 + r*cosPsi*(r1l1 - r2l2))) + (6*r_2*cosPsi*ln) + (r_2*r*((3*cosPsi*cosPsi) - 1)*( ((r1+l1)/(l1*lnbot)) - ((r2+l2)/(l2*lntop)) )));
...
/* Conversion factor */
/* 1 m/s^2 = 10^5 mGal */
double GyCalculator2D::si2mgal = 100000;
