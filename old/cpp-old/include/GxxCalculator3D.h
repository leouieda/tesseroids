#ifndef _GXXCALCULATOR3D_H_
#define _GXXCALCULATOR3D_H_
...
class GxxCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GxxCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GxxCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Gxx <spherical> component of the Gravity Gradient Tensor using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gxx <spherical> component of gravity gradient tensor <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = ( rl_2*cosPhil*(((((3*rrl*rrl*cosPsiPhi*cosPsiPhi)/(l_3*l*l)) - ((rrl*cosPsi)/(l_3))) + r*(((rl*cosPsi) - r) / (l_3))) / r_2 ) );   
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GxxCalculator3D::si2eotvos = 1000000000;
