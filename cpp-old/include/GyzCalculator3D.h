#ifndef _GYZCALCULATOR3D_H_
#define _GYZCALCULATOR3D_H_
...
class GyzCalculator3D: public FieldCalculator {
    ...
    private:
        ...
        inline double KV(double rl, double phil, double lambl, double r, double phi, double lamb);  
    ...
    public:
        ...
        GyzCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GyzCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Gyz <spherical> component of the Gravity Gradient Tensor using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gyz <spherical> component of gravity gradient tensor <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = rl_2*cosPhil*( (((r*rl_l3*cosPsiLamb)/r) - (rl_l3*cosPsiLamb*((3*r*((rl*cosPsi) - r)/(l*l)) + 1))) / (r*cosPhi) );  
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GyzCalculator3D::si2eotvos = 1000000000;
