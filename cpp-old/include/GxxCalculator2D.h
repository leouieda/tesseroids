#ifndef _GXXCALCULATOR2D_H_
#define _GXXCALCULATOR2D_H_

/* ****************************************************************************************** */
/* ********************************* GxxCalculator2D **************************************** */
/* ****************************************************************************************** */
/* **************** Calculate the Gxx component of the gravity gradient tensor ************** */
...
class GxxCalculator2D: public FieldCalculator {
    ...
    public:
        ...
        GxxCalculator2D(int newLatO, int newLonO){
            latOrder = newLatO;
            lonOrder = newLonO;        
            boundsSet = false;
            hightSet = false;
        ...
        GxxCalculator2D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO){            
            latOrder = newLatO;
            lonOrder = newLonO;        
            W = newW;
            E = newE;
        ...
        /* Calculate the Gxx <spherical> component of the Gravity Gradient Tensor using analytical integration */
        ...
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gxx <spherical> component of gravity gradient tensor <2D GLQ>");
                throw ex;
            }
                    ...
                    for(lamb=Wrad, clon=0; /*lon<=Erad && */clon<nlon; lamb+=lonStep, clon++){
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){         
                                ...
                                t3 = 3*r_2*cosPsiPhi_2*(2*(r1l1-r2l2) + r*cosPsi*((r1l1*r1l1/l1) - (r2l2*r2l2/l2)));
                                t4 = 3*r*cosPsi*(l2 - l1 + cosPsi*(r*r1l1 - r*r2l2));
                                t5 = 6*r_2*ln*(cosPsiPhi_2 - cosPsi*cosPsi);
                                t6 = r_2*r*cosPsi*(12*cosPsiPhi_2 - cosPsi_2_1)*((r1+l1)/(l1*lnbot) - (r2+l2)/(l2*lntop));
                                t7 = r_2*r_2*cosPsiPhi_2*cosPsi_2_1*((r1*lnbot - (r1+l1)*(r1l1*lnbot + r1 + l1))/(l1*l1*lnbot*lnbot) - (r2*lntop - (r2+l2)*(r2l2*lntop + r2 + l2))/(l2*l2*lntop*lntop));
                                KVphi_2 = 0.5*( t2 - t1 + t3 - t4 + t5 + t6 - t7 );
                                /* KVr */
                                KVr = ( (r2*l2) - (r1*l1) + (3*r*cosPsi*(l2-l1)) + (r_2*cosPsi_2_1*ln) - ((r2l2*r2_2) + ((r1l1*r1_2))) ) / r;
           
                                KV = cosPhil*(KVphi_2 + r*KVr) / r_2;
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GxxCalculator2D::si2eotvos = 1000000000;
