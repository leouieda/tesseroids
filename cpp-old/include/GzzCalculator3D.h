#ifndef _GZZCALCULATOR3D_H_
#define _GZZCALCULATOR3D_H_
...
class GzzCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GzzCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GzzCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Gzz Component (Spherical) of the Gravity Gravient Tensor using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("Gzz <spherical> component of gravity gradient tensor <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = rl_2*cosPhil*( (((3*rlcosPsi_r*rlcosPsi_r)/(l*l)) - 1) / l_3 );   
                                    /* Sum the KV with the wights */
...
/* Conversion factor */
/* 1 /s^2 = 10^9 Eotvos */
double GzzCalculator3D::si2eotvos = 1000000000;
