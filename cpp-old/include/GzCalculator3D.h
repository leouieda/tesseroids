#ifndef _GZCALCULATOR3D_H_
#define _GZCALCULATOR3D_H_
...
class GzCalculator3D: public FieldCalculator {
    ...
    public:
        ...
        GzCalculator3D(int newLatO, int newLonO, int newRO){
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            boundsSet = false;
        ...
        GzCalculator3D(double newW, double newE, double newS, double newN, double newZ, int newNlon, int newNlat, int newLatO, int newLonO, int newRO){            
            latOrder = newLatO;
            lonOrder = newLonO;
            rOrder = newRO;
            W = newW;
        ...
        /* Calculate the Vertical Component of the Gravity Field using a 3D Gauss-Legendre Quadrature */
        Data* calculate(vector<ModelElement>& mod_vector) throw (GridParamNotSetException){
            ...
            if(!paramsSet()){
                GridParamNotSetException ex("vertical <spherical> component of gravity <3D GLQ>");
                throw ex;
            }
                        ...
                        for(i=0; i<latOrder; i++){
                            ...
                            for(j=0; j<lonOrder; j++){
                                ...
                                for(k=0; k<rOrder; k++){
                                    ...
                                    KV = rl_2*cosPhil*((r-(rl*cosPsi)) / (l*l*l));   
...
/* 1 m/s^2 = 10^5 mGal */
double GzCalculator3D::si2mgal = 100000;
